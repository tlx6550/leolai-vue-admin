...
<script type="text/ecmascript-6">
  import {fetchList,createArticle,updateArticle,fetchPv} from '@/api/articles'
  ...
  const calendarTypeKeyValue = calendarTypeOptions.reduce((total, cur) => {
    total[cur.key] = cur.display_name
    return total
  }, {})
  ...
  data(){
   ...
   computed: {
    ...
    filters:{
      ...
      typeFilter(item){
        return calendarTypeKeyValue[item]
      }
    },
   methods: {
     handleFilter(){
     ...
     handleDownload(){
         // 原理https://stackoverflow.com/questions/14964035/how-to-export-javascript-array-info-to-csv-on-client-side
       let arr = [],newListKey=[]
       this.list.forEach((item)=>{
        ...
        newListKey = inarr.filter((key)=>{
           if(key==='id')return false
           if(key==='auditor')return false
           if(key==='forecast')return false
           if(key==='timestamp')return false
           return true
         })
         newListKey.forEach(newkey=>{
           onelist.push(item[newkey])
         })
         arr.push(onelist)
       })
      console.log(arr)
